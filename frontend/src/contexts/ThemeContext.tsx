...
export function ThemeProvider({ 
  ...
  const [theme, setTheme] = useState<ThemeContextType>({
    primaryColor: initialTheme?.primaryColor || '#3f5e78',
    secondaryColor: initialTheme?.secondaryColor || '#f5f5f5',
    accentColor: initialTheme?.accentColor || '#007bff',
  ...
  useEffect(() => {
    if (initialTheme) {
      ...
      document.documentElement.style.setProperty('--primary-color', initialTheme.primaryColor || '#3f5e78');
      document.documentElement.style.setProperty('--secondary-color', initialTheme.secondaryColor || '#f5f5f5');
      document.documentElement.style.setProperty('--accent-color', initialTheme.accentColor || '#007bff');
