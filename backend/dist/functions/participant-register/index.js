...
const handler = async (event) => {
    try {
        ...
        const validation = (0, utils_1.validateParticipantData)(body);
        if (!validation.valid) {
        ...
        if (!eventData.registration.enabled) {
            return (0, utils_1.createResponse)(410, {
                success: false,
        ...
        if (eventData.registered_count >= eventData.capacity) {
            return (0, utils_1.createResponse)(409, {
                success: false,
                error: { code: 'CAPACITY_FULL', message: 'Event is at full capacity' }
            });
        }
        // Check if email already registered for this event
        const existingQuery = await docClient.send(new lib_dynamodb_1.QueryCommand({
        ...
        const participantId = `part_${(0, uuid_1.v4)()}`;
        // Get current participant count for registration number
        const countQuery = await docClient.send(new lib_dynamodb_1.QueryCommand({
            TableName: PARTICIPANTS_TABLE,
        ...
        const registrationNumber = (0, utils_1.generateRegistrationNumber)(tenantId, (countQuery.Count || 0) + 1);
        // Generate QR code data
        const qrData = (0, utils_1.generateQRData)(tenantId, body.event_id, participantId);
